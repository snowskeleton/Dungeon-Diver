...
export class WeaponSlotState extends Observable implements WeaponSlotView {
  ...
  @tracked("string") displayName: string = "";
  @tracked("int32") tint: number = -1;
  @tracked("float32") damage: number = 0;
  @tracked("uint16") attackCooldownMs: number = 0;
  @tracked("float32") attackForce: number = 0;
  // Post-fold projectile stats; all zero for a melee weapon.
  @tracked("float32") ammoDamage: number = 0;
  @tracked("float32") ammoSpeed: number = 0;
  @tracked("uint8") ammoPierce: number = 0;
  @tracked("float32") ammoKnockback: number = 0;
  @tracked(["string"]) modLabels = new ObservableList<string>();
}
