...
server/src/
  ...
  schema/GameState.ts       ← root schema: MapSchema of players + enemies + projectiles + shops + rewards + supplies (keyed by room/pedestal id), floor number, `paused` flag. `supplies` reuses RewardState (kind "weapon") — the floor-1 supply-room pedestals, one per player
