...
- **OO is enforced, not preferred. Game content is classes, never flat config files.** Every kind of game content — enemies, bosses, weapons, ammo, upgrades, characters — is modelled as a class hierarchy whose stats and behaviour are compiler-checked getters on the subclass, collected in a plain array of classes/instances (`REGULAR_ENEMIES` / `BOSSES` / `WEAPONS` / `AMMO_CLASSES` / `UPGRADES` / `CHARACTERS`). A flat `Config` object or a `Record<Id, Config>` "registry" describing what something *is* is **not allowed** — if you find one, it is a bug to convert, no matter how simple it looks. (The one flat config that survived this rule, `CHARACTER_REGISTRY`, was deleted and rewritten as `Character` subclasses — do not reintroduce its shape.) The **only** id-keyed maps permitted are: id→instance/template lookups *derived from* the content array for wire-referenced ids (`WEAPON_REGISTRY`, `AMMO_REGISTRY`, `getCharacter`), and thin client-side id→visual-def wiring tables that are compiler-checked to cover every id. Both hold real objects for lookup; neither is where stats or behaviour are *authored*.
...
shared/src/
  ...
  characters/          ← characters are OO like enemies/weapons: `Character` base (abstract, stats as getters in base.ts) → one concrete class per class (Knight/Rogue/Ranger/Mage in <Class>.ts), each overriding `id`, `name`, `maxHp`, `speed`, and `usableCategories` (the weapon CATEGORIES this class may wield — D9/D18, the restriction declared ON the class, not in a lookup table). NO CharacterConfig, NO CHARACTER_REGISTRY (that flat-config design was killed — see the engineering note). Players have NO default weapon — the first one is rolled at the floor-1 supply room. index.ts exports `CHARACTERS` (the array of instances, mirroring REGULAR_ENEMIES) + `CHARACTER_CLASSES` + `getCharacter(cls)` (id→instance, DERIVED from CHARACTERS) + the query helpers that DERIVE everything from usableCategories: `canClassUseWeapon`, `firstRollCategories`/`firstRollWeaponIds` (a class's UNIQUE categories = its first-weapon pool), `partyRollableWeaponIds` (D10 loot filter). base.ts also holds `CHARACTER_TYPES` (12 humanoid skins) with the CharacterType union DERIVED from it, so the runtime list and the union can't drift; index.ts also exports `resolveCharacterClass`/`resolveCharacterType`, the validators every client-supplied id must pass through
  ...
  economy.ts           ← the run economy as DERIVED constants: one shared party gold purse fed by enemy coin drops, spent at shop/shrine. A floor's gold is BUDGETED (not per-enemy priced) and split across what spawned by each enemy's goldWeight; tier costs are fixed and the per-floor budget flat, so depth scales shop QUALITY not price. No id→gold table. See roadmap.html "Currency"
...
server/src/
  ...
  rooms/LootDirector.ts     ← everything reward-shaped: shops, shrine/boss/challenge offers, the single-reward room-clear pedestals, the maze chests, the rolls behind them, and the validate-then-grant half of the buy/offerPick/claimReward/chestOpen messages (GameRoom's handlers are one line each)
  ...
  pathfinding/FlowFieldSystem.ts ← shared enemy chase pathing: once per tick, BFS-floods a distance field toward the players so every enemy follows a gradient instead of running its own A*. Enemy.tick reads it; SpawnDirector wires it in via setFloor(..., flowField). See docs/pathfinding.md
  ...
  schema/RewardState.ts     ← a single-reward pedestal dropped where a room's last enemy fell. No choice and no cost: interact and the one reward (weapon/upgrade/gold) is yours. `kind` picks the payload; weapon `mods` are UNDECORATED (server-only) because mods are behaviour, same as OfferChoiceState.mods
  schema/ChestState.ts      ← a treasure chest at the DEEP END of a maze room (LootDirector.mazeDeepestTile), placed at generation and openable anytime — solving the maze IS the gate. No choice and no cost. weaponId/mods are UNDECORATED (server-only) both because mods are behaviour AND because syncing them would spoil the surprise
  ...
  schema/GameState.ts       ← root schema: MapSchema of players + enemies + projectiles + shops + offers + rewards + supplies + chests + challenges + coins (keyed by room/pedestal id), floor number, `paused` flag. `supplies` reuses RewardState (kind "weapon") — the floor-1 supply-room pedestals, one per player
...
client/src/
  ...
  launch.ts                 ← pickLoadout() = the character picker (class + skin) only. Run from the LOBBY on a player who already exists, so it pre-selects the current pick. There is NO weapon step anymore — players spawn empty-handed and claim their first weapon at the floor-1 supply room
  ...
  entities/RewardPedestalEntity.ts ← in-world view of a single-reward pedestal (dropped where a room's last enemy fell). Near-sibling of OfferPedestalEntity; ghosts out once claimed. Not an Entity (no HP bar)
  entities/InteractPrompt.ts ← a small world-space "press <key>" hint floated above any interactable (shop/reward pedestal, maze chest) when a local player is in range; key label from options/keybindings
  ...
  ui/CharacterPicker.ts     ← join-time class + skin chooser (DOM overlay). The only loadout picker now — the join-time weapon chooser was removed when players went empty-handed-at-spawn (first weapon comes from the floor-1 supply room)
  ...
  options/gameOptions.ts    ← OPTION_FIELDS + localStorage-backed GameOptions (camera zoom, hitbox overlay, controls hint, melee comboWindowMs/chargeHoldMs)
  options/keybindings.ts    ← the rebindable key map + promptKeyLabel() (the label InteractPrompt shows), persisted like the other options
  ui/menuDom.ts             ← the ONE stylesheet + builders (`el`/`button`/`menuPanel`/`selectOne`/`addStyle`) behind every full-screen DOM overlay — browser, lobby, pause, the character picker, inventory, offer, confirm, FieldPanel. A panel's own file adds ONLY what makes it different (portrait cropping, weapon tabs, the confirm dialog's red frame), via `addStyle`
...
**The tileset is generated CODE that SAMPLES real art.** `assets/generate-dungeon-tiles.js` builds **both** `assets/dungeon-tiles.png` and `client/src/map/tilesetFrames.generated.ts` in one run. Walls and the floor are sampled from the **Super Overhead Adventure 2 dungeon sheet** (the same art pack the characters, enemies and bosses came from — so the environment matches the sprites); the small/animated/absent tiles (stairs, trap, boss passage, fire, slime, wall shadow, door portcullis) are drawn procedurally. The source sheet (`Environments/Dungeon.png`) lives OUTSIDE the repo like the SOA2 character import — the committed PNG + table are what the game loads; you only need the source to re-run, and can point at yours with `SOA2_DUNGEON=…`. Re-run with `npm run assets:tiles`; set `TILE_PREVIEW=/path.png` to also dump a sample room per theme. Never repaint the PNG — it is build output.

**We use ONE colour set: the blue/cyan set** (the sheet's far-left 512px block). The author's own composition — `Examples/Zaldo/Example Dungeon.png` — is the reference for how the tiles compose, and prior passes got the floor wrong twice (busy panels, then a flat fill). The floor is a **dark-blue flagstone**: dark stones inset in navy mortar. Three things carry it:

- **Three stone sizes, mixed per room.** `tiny` (a 2×2 of small stones in one cell), `regular` (one ~26px stone per cell), `large` (48px stones). 48 and the 32px cell realign every 96px, so a `large` block is **3×3 cells** holding a 2×2 grid of 48px stones, baked as nine frames (row-major) the renderer lays as a group — the stones straddle cell boundaries, invisible on a floor. (48px, not the old 64px 2×2 slab: a stone bigger than one cell must span whole cells, and 3-cell blocks are the step below the 2-cell one.) `TileRenderer` picks a size PLAN per room from its id — uniform, a small-stone rim around large inner slabs, or a size split down the middle — so a room reads as different from its neighbour through stone SIZE (we're on one colour set, so not colour). A `large` cell whose 3×3 block would be clipped (room edge, or a rim boundary) falls back to `regular` so no partial slabs appear.
- **Hand-placed irregularity.** Every stone is drawn (not blitted) as a rounded rect in the example's exact two-colour palette (`STONE`/`MORTAR`), a touch smaller than its slot, nudged off-centre, and rotated a few degrees. The generator bakes ~14 variants each of tiny/regular + 8 large sets; the renderer scatters them by a stable hash so no two neighbouring slots look placed the same and the floor never reshuffles on a rebuild.
- **Interior obstacles are blocks, not brick.** A `TILE.WALL` inside a room's INTERIOR (not the perimeter ring, not the void between rooms) is a designer-placed cover block / "tetris piece" — it renders as one of the sheet's raised cyan blocks (`BLOCK_FRAMES`, socket/motif blocks with a baked shadow skirt), exactly as the example uses them. Only the room's structural shell autotiles as brick.

Why generated rather than a hand-cut atlas:

- **Walls autotile, and the 47-tile blob is complete by construction.** A wall's frame comes from which of its eight neighbours are also wall, so edges, outer corners (bitten to read as rounded), inner corners, end-caps and 1-tile corridors all fall out of the geometry — no room shape can produce an arrangement the sheet lacks. Only the 4 cardinals plus the diagonals *between two set cardinals* matter, which collapses 256 arrangements onto 47; the generator enumerates the canonical set and emits the whole 256-entry lookup, so `TileRenderer` computes a raw mask and indexes it directly. Every wall is one real cyan brick tile (`WALL_BRICK`) with directional edge shading + the corner bite composited on.
...
- `WALL_BRICK` / `BLOCK_SRC` are sheet coordinates; the flagstone stones are drawn procedurally in the sampled palette. Every room type uses the same blue floor for now; a second colour set can be added later.

Walkable tiles get the floor drawn underneath and the special (stairs/trap/boss/fire/slime) on top, so a stairwell sits **in** the room's floor rather than replacing a square of it. The barrier over a locked doorway is a tileset frame too (an iron portcullis) — it used to be a red rectangle drawn with `Graphics` at boot, which is why locked doors read as a debug overlay.

**Content styles — OO, never flat config (enforced, see below).** **Enemies, bosses, weapons, ammo, upgrades, AND characters are object-oriented**: one subclass each, stats as compiler-checked getters resolved up an `extends` chain, listed in a plain array of classes (`REGULAR_ENEMIES` / `BOSSES` / `WEAPONS` / `AMMO_CLASSES` / `UPGRADES` / `CHARACTERS`) — no `ENEMY_REGISTRY`, no `CHARACTER_REGISTRY`, no generic `Goo`, no id→config table. Enemies/bosses put *behaviour* on the class (`server/src/entities/enemies` + `/bosses`); weapons (`shared/src/weapons`, three-level chain `Weapon → Sword/Bow/Staff/… → the weapon`), ammo (`shared/src/ammo`, `Ammo → Arrow/Bolt/Boomerang → the ammo`), and characters (`shared/src/characters`, `Character → Knight/Rogue/Ranger/Mage`) currently put only *stats* there, but being real classes means a specific one can grow a bespoke method later without reshaping anything. `WEAPON_REGISTRY` / `AMMO_REGISTRY` (id→template) and `getCharacter` (id→instance) are *derived* from `WEAPONS` / `AMMO_CLASSES` / `CHARACTERS` because all are referenced by id across the wire — a genuine lookup need, not a config shortcut. Client visuals live in parallel registries (`client/src/characters`, `client/src/enemies`, `client/src/weapons`) — thin id→visual-def wiring tables, compiler-checked to cover every id, which is a legitimate use distinct from stats-as-flat-config. Clients pass `characterClass`/`characterType` as join options, synced on `PlayerState` — both are UNTRUSTED and go through `resolveCharacterClass`/`resolveCharacterType` rather than being cast (an unknown class used to crash `onJoin`).
...
**Weapon access is class-restricted (D9/D18), declared on the class.** Each `Character`'s `usableCategories` getter lists the weapon categories that class may wield; the four melee categories (sword/axe/spear/rapier) are on every list (the shared backbone), while each class also owns a few UNIQUE categories (Knight → hammer/mace, Mage → staff, Ranger → bow/crossbow, Rogue → dagger/thrown). There is **no** category→class lookup table — `canClassUseWeapon`, `firstRollCategories`, and `partyRollableWeaponIds` all DERIVE from the per-class lists. `Player.canEquip` gates every weapon-granting path (shop/offer/reward/supply/chest); an incompatible pickup grants nothing and returns a reason string the GameRoom relays as `loot_error` (client flashes it). Loot rolls are filtered to what the present party can use (D10). **Players spawn with no weapon** — their first is claimed at the floor-1 supply room.

**Room type system**: `dungeonGenerator.ts` assigns a `RoomType` (`"combat" | "maze" | "boss" | "shop" | "shrine" | "wave" | "timed" | "dark" | "supply"`) to each room during generation — before carving, so the carve function varies by type. The run's **start room is always `"supply"`**: a cover-free staging room, never populated, and on **floor 1 only** it drops one weapon pedestal per player (`LootDirector.spawnSupply`, rolled from each player's `firstRollWeaponIds`). Boss is placed first (random non-start room), then all others get a weighted roll (44% combat / 6% timed / 4% dark / 8% wave / 16% maze / 15% shop / 7% shrine). `RoomData.type` carries the type through to `GameRoom`, which uses it to decide enemy spawning (the reward rooms in `NO_RABBLE_ROOM_TYPES` — including `supply` — get none; the boss room gets a single boss, not the usual rabble; **the start room never gets enemies** so players aren't jumped on load — the one exception is a degenerate single-room floor where start === exit). Floor loot placement (`GameRoom.placeFloorLoot`) is deferred to `startRun`/`advanceFloor`, not `initFloor`, so the D10 party filter sees the settled party. Boss passageway tiles are overwritten with `TILE.BOSS_FLOOR` (gold, breathing animation) after connections are built. **Every non-reward room drops a single reward pedestal on clear** (`LootDirector.dropRoomReward`, fired where the last enemy fell) — the old chest room type was retired in favour of this, so loot isn't gated to a rare room. **Maze rooms additionally hold a treasure chest at their deepest tile** (`mazeDeepestTile` — BFS from the maze's doorways), placed at generation and openable anytime, so solving the maze is its own reward on top of the clear pedestal. See [docs/upgrades.md](docs/upgrades.md) § Reward pedestals.
...
| Knob | File |
|---|---|
| Player/class (maxHp, speed, usable weapon categories) | `shared/src/characters/<Class>.ts` (getters on the `Character` subclass) |
...
| Melee combo (per-swing damage/knockback multipliers) | `comboNDamageMult`/`comboNKnockbackMult` getters on the `Weapon` base or a category base (`shared/src/weapons/base.ts`), overridable per weapon — all show in the weapon-balance tool. The combo grace window is a client Option (`comboWindowMs`, sent to the server); default in `DEFAULT_COMBO_WINDOW_MS` (`shared/src/types.ts`). See [docs/animation.md](docs/animation.md) |
| Hard (charged/held) melee swing | `hardDamageMult`/`hardKnockbackMult` getters on the `Weapon`/category base (`shared/src/weapons/base.ts`, default = combo finisher), in the weapon-balance tool; the hold-to-charge threshold is a client Option (`chargeHoldMs`, default `DEFAULT_CHARGE_HOLD_MS`). A tap swings immediately; holding past the threshold arms the hard swing, which fires on release — see [docs/animation.md](docs/animation.md) |
...
| Room-clear rewards (weapon/upgrade/gold mix, gold payout) | `ROOM_REWARD_WEIGHTS` / `ROOM_REWARD_GOLD` in `server/src/rooms/LootDirector.ts` |
| Maze chests (gold rarity, how many modifiers each tier rolls) | `GOLD_CHEST_CHANCE` / `*_CHEST_MODS` in `server/src/rooms/LootDirector.ts` |
...
| Store (pedestal count, gold price tiers, buy radius) | `server/src/rooms/LootDirector.ts` (`SHOP_ITEM_COUNT` / `SHOP_TIERS` / `BUY_RADIUS`) |
...
3. Add its frame in `assets/generate-dungeon-tiles.js` — either a `draw*()` function (procedural, like the trap) or a `blitSource()` from the SOA2 sheet — plus an entry in `special`; run `npm run assets:tiles`, then add a `case` for it in `TileRenderer.buildMap`'s switch. Don't hand-paint `dungeon-tiles.png` — it is regenerated
...
- **New class** (stats + usable weapon categories): a `Character` subclass in `shared/src/characters/<Class>.ts` (override `id`/`name`/`maxHp`/`speed`/`usableCategories`) → add to the `CharacterClass` union in `base.ts` + append an instance to `CHARACTERS` in `index.ts`. No client-side registry entry needed — attack FX comes from the weapon.
