...
**We use ONE colour set: the blue/cyan set** (the sheet's far-left 512px block). The author's own composition — `Examples/Zaldo/Example Dungeon.png` — is built from this set and is the reference for how the tiles compose. Two things that reference makes clear (and that a prior pass got wrong): the floor is a **flat solid dark-blue** fill (`FLOOR_SRC`), and the decorated block/water/panel tiles are 3D **objects that sit on** the floor and cast shadows, **not** floor fill — tiling them as the floor is what made the previous version look busy and wrong. Every room type uses the same blue floor for now; the `THEME_NAMES` keys survive so a second colour set can be slotted in per room type later without touching the renderer.
