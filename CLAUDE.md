...
**Content styles.** **Enemies, bosses, and weapons are object-oriented**: one subclass each, stats as compiler-checked getters resolved up an `extends` chain, listed in a plain array of classes (`REGULAR_ENEMIES` / `BOSSES` / `WEAPONS`) — no `ENEMY_REGISTRY`, no generic `Goo`, no id→config table. Enemies/bosses put *behaviour* on the class (`server/src/entities/enemies` + `/bosses`); weapons (`shared/src/weapons`, three-level chain `Weapon → Sword/Bow/Staff/… → the weapon`) currently put only *stats* there, but being real classes means a specific weapon can grow a bespoke method later without reshaping anything. `WEAPON_REGISTRY` (id→template) is *derived* from `WEAPONS` because weapons are referenced by id across the wire — a genuine lookup need, not a config shortcut. Still plain config in `shared/` registries: **characters** (`CHARACTER_REGISTRY` — per-class maxHp/speed/defaultWeaponId) and **ammo** (`AMMO_REGISTRY` — projectile stats); add one by adding a config + registry entry. Client visuals live in parallel registries (`client/src/characters`, `client/src/enemies`, `client/src/weapons`). Clients pass `characterClass`/`characterType` as join options, synced on `PlayerState`.
