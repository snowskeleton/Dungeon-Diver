...
- **Three stone sizes, mixed per room.** `tiny` (a 2×2 of small stones in one cell), `regular` (one per cell), `large` (one stone across a 2×2 cell block, baked as four quadrant frames the renderer lays as a group). `TileRenderer` picks a size PLAN per room from its id — uniform, a small-stone rim around large inner slabs, or a size split down the middle — so a room reads as different from its neighbour through stone SIZE (we're on one colour set, so not colour). A `large` cell whose 2×2 block would be clipped (room edge / odd dimension) falls back to `regular` so no half-slabs appear.
