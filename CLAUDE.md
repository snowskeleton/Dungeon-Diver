...
- **Room types are legible at the locked 2× zoom** — the Floor 6 "texture variety" item. Since walls are uniform, the **FLOOR** carries room identity: `themeFor(RoomType)` is an **exhaustive switch** (a new room type is a compile error, not a silent fallback to stone) mapping each type to one of six floor themes (stone / maze / shop / shrine / chest / boss). Combat/wave/timed/dark deliberately share `stone`. This pack's only floor tiles are busy decorated *panels*, which tile into a dense grid; so each theme is two frames — a **calm base** (the panel's own stone colour, sampled from the art, laid flat with a faint seam) for ~5 of 6 tiles, and the **real decorated panel** scattered in as the 6th by a per-tile hash (stable across rebuilds, or the floor reshuffles underfoot). `FLOOR_THEME_SRC` / `WALL_BRICK` in the generator are the source coordinates, verified by preview against the sheet.
