...
**We use ONE colour set: the blue/cyan set** (the sheet's far-left 512px block). The author's own composition — `Examples/Zaldo/Example Dungeon.png` — is built from this set and is the reference for how the tiles compose. Two things that reference makes clear (and that prior passes got wrong): the floor is a **dark-blue flagstone** (dark stones with darker mortar joints, a few near-identical `FLOOR_TILES` variants scattered so it isn't a mechanical grid — **not** a flat solid fill, and **not** the decorated panels), and the decorated block/water/panel tiles are 3D **objects that sit on** the floor and cast shadows, **not** floor fill. The floor tiles are **lifted from the example composition** (`EXAMPLE_PATH`), because the dungeon sheet's own floor tiles don't sit on a clean 16px sub-grid; walls and specials still come from the sheet. Every room type uses the same blue floor for now; the `THEME_NAMES` keys survive so a second colour set can be slotted in per room type later without touching the renderer.
...
- `WALL_BRICK` / `FLOOR_SRC` in the generator are the source tile coordinates, verified by preview against the sheet.
