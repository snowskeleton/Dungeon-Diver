...
- `generate-dungeon-tiles.js` — **the dungeon tileset.** Builds both `assets/dungeon-tiles.png` and `client/src/map/tilesetFrames.generated.ts` (the frame table the renderer indexes) in one run. Walls and floors are **sampled from the Super Overhead Adventure 2 dungeon sheet** (`Environments/Dungeon.png` — the same pack the characters/enemies/bosses came from); stairs, trap, boss passage, fire, slime, wall shadow and the door portcullis are drawn procedurally (small, animated, or absent from the pack). Run with **`npm run assets:tiles`** (regenerates + syncs); add `TILE_PREVIEW=/tmp/rooms.png` to also dump a sample room per theme. The source sheet lives **outside the repo** (like the SOA2 character import) — the committed PNG + table are what the game loads, so you only need the source to re-run; override its path with `SOA2_DUNGEON=/path/to/Dungeon.png`. **Only `pngjs` is required, which IS a project dependency** — unlike the `sharp` scripts below, this runs with no extra install. Source tile coordinates are `WALL_BRICK` (the one universal wall brick) and `FLOOR_THEME_SRC` (one decorated floor panel per room theme); to re-theme a room, point its entry at a different panel on the sheet. Why generated rather than a hand-cut atlas: the 47 wall-blob frames have to be mutually consistent or walls grow wrong edges at some room shape nobody tested, and the frame indices have to match the renderer — both guaranteed by construction. See the tileset section in [../CLAUDE.md](../CLAUDE.md).
