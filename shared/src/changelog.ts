...
export const CHANGELOG: ChangelogVersion[] = [
  {
    ...
    changes: [
      ...
      { kind: "feature", text: "Full controller support: P1 can now play solo on a gamepad (left stick moves, A attacks/confirms, X picks up and interacts, B backs out, Y uses your movement ability, LB/RB switch weapons, View opens the inventory, Start opens the pause menu) and can swap between the pad and keyboard mid-run. Each couch player claims the next controller" },
      { kind: "feature", text: "B is now Back everywhere on a controller — the standard console convention — so it cancels out of any menu; it's a rebindable control (Back / Cancel) in the Controls screen, and A/Select and X/Pickup are now distinct buttons" },
