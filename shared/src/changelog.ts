...
export const CHANGELOG: ChangelogVersion[] = [
  {
    ...
    changes: [
      ...
      { kind: "feature", text: "Full controller support: P1 can now play solo on a gamepad (left stick moves, RB attacks, X picks up and interacts, B backs out, Y uses your movement ability, LB/RT switch weapons, View opens the inventory, Start opens the pause menu) and can swap between the pad and keyboard mid-run. Each couch player claims the next controller" },
      { kind: "feature", text: "B is now Back everywhere on a controller — the standard console convention — so it cancels out of any menu; it's a rebindable control (Back / Cancel) in the Controls screen. Menu Select/Confirm (A) is now its own fixed control, separate from your in-game Attack, so you can rebind Attack freely without changing how you confirm menus" },
      ...
      { kind: "feature", text: "Controls can be rebound for BOTH devices: the rebind screen now has a Keyboard/Mouse column and a Controller column, you can navigate and rebind it with the controller itself, and on-screen button prompts show the right glyph for whichever device you last used. The reserved Pause control (Esc / Start) is shown greyed-out so you can see it even though it can't be changed" },
      ...
      { kind: "feature", text: "Enemies now telegraph and attack instead of just bumping you: the beasts hold and swing their own weapons with a wind-up, the eye-bat spirals in and dives, the frog crouches then leaps and slams, the smushroom swells up as a warning before gassing you with a lingering cloud (and a parting cloud when it dies), and the snake coils then lunges" },
