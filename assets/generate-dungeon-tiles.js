...
/** Large flagstones are 48px — bigger than `regular` (a full cell) but smaller
 *  than the old 2×2. 48 and the 32px cell line up every 96px, so a 3×3-cell
 *  (96px) block holds a 2×2 grid of 48px stones and tiles cleanly. Baked as nine
 *  32px frames (row-major) the renderer lays in a 3×3 group; the stones straddle
 *  cell boundaries, which is invisible on a floor. */
function drawLargeBlock(seed) {
  const surf = new Tile(96);
  surf.fill(MORTAR);
  const g = stoneRng(seed);
  for (const [cx, cy] of [[24, 24], [72, 24], [24, 72], [72, 72]]) {
    stampStone(surf, cx + g.j(2.5), cy + g.j(2.5), g.range(42, 46), g.range(42, 46), g.j(4.5), 6);
  }
  const quads = [];
  for (let by = 0; by < 96; by += 32) for (let bx = 0; bx < 96; bx += 32) quads.push(surf.quadrant(bx, by));
  return quads; // 9 frames, row-major
}
...
const FLOOR_TINY = [];
const FLOOR_REGULAR = [];
const FLOOR_LARGE = []; // each entry is 9 frame indices (a 3×3-cell block, row-major)
...
for (let i = 0; i < 8; i++) {
  FLOOR_LARGE.push(drawLargeBlock(0xb500 + i * 3266489917).map((q) => push(q)));
}
  ...
  plans.forEach((plan, ti) => {
    ...
    for (let ry = 0; ry < TH; ry++) {
      for (let rx = 0; rx < TW; rx++) {
        ...
        if (size === "large") {
          const bcol = Math.floor(irx / 3), brow = Math.floor(iry / 3);
          const q = (iry % 3) * 3 + (irx % 3);
...
console.log(
  ...
  `${FLOOR_TINY.length + FLOOR_REGULAR.length + FLOOR_LARGE.reduce((n, s) => n + s.length, 0)} floor, ` +
